...
if (process.argv[2] === 'publish') {
  if (process.env.NODE_AUTH_TOKEN !== 'fixture-auth') throw new Error('publish credential missing');
  ...
  if (process.env.NODE_AUTH_TOKEN) throw new Error('credential leaked to archive retrieval');
...
const curl = `
import fs from 'node:fs';
if (process.env.NODE_AUTH_TOKEN) throw new Error('credential leaked to registry probe');
...
function runCase(options) {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [path.join(projectRoot, "scripts/publish-npm-package.mjs"), "--version", "0.5.0", "--package-dir", path.join(fixture.directory, "package"), "--dist", path.join(fixture.directory, "dist")], {
      ...
      env: fixtureEnvironment(fixture, { NODE_AUTH_TOKEN: "fixture-auth", FIXTURE_MODE: options.mode ?? "success" }),
    });
    ...
    assert.doesNotMatch(result.stdout + result.stderr, /fixture-auth/);
...
for (const options of [
  { success: true }, { success: true, existing: true }, { success: true, mode: "ambiguous" },
