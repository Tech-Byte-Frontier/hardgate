...
includesAll(selfGate, ["check --format agent", "check --checks policy", "--coverage-report \"$HARDGATE_COVERAGE_REPORT\" --format agent", "HARDGATE_COVERAGE_REPORT=.hardgate/evidence/coverage.lcov", "evidence cargo-mutants", "--file src/engines/budgets.rs", "replace check_measured_budgets", "--mutation-report \"$HARDGATE_MUTATION_REPORT\"", "HARDGATE_BINARY=\"$BINARY\" node scripts/check-consumer-matrix.mjs"], "self gate");
