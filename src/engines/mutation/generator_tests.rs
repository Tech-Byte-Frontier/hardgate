...
fn test_python_mutation_generation() {
    ...
    assert!(
        !mutants.is_empty(),
        "Python function should have mutation points"
    );
    ...
    assert!(
        bool_mutants
            .iter()
            .any(|m| m.original == "and" && m.replacement == "or")
    );
    assert!(
        bool_mutants
            .iter()
            .any(|m| m.original == "is not" && m.replacement == "is")
    );
    assert!(
        bool_mutants
            .iter()
    ...
    assert!(
        bool_mutants
            .iter()
            .any(|m| m.original == "False" && m.replacement == "True")
    );
}
