...
pub(super) fn rust_coverage(options: &EvidenceOptions, output: PathBuf) -> Result<CommandSpec> {
    ensure!(
...
pub(super) fn rust_mutation(
    ...
    if let Some(partition) = partition {
        super::rust_scope::validate(options, root, partition)?;
    }
    let mut tokens = cargo_prefix(options);
    ...
    if let Some(partition) = partition {
        for source in &partition.sources {
            tokens.extend(["--file".into(), source.display().to_string()]);
        }
    }
    Ok(CommandSpec {
        tokens,
