...
: "${RUST_TEST_THREADS:=1}"
HARDGATE_WORKLOAD_JOBS=$(node scripts/check-resource-boundary.mjs --jobs)
export HARDGATE_WORKLOAD_JOBS
for guard_key in CARGO_BUILD_JOBS RUST_TEST_THREADS RAYON_NUM_THREADS GOMAXPROCS OMP_NUM_THREADS OPENBLAS_NUM_THREADS MKL_NUM_THREADS CMAKE_BUILD_PARALLEL_LEVEL; do
  ...
  eval "guard_value=\${$guard_key:-$HARDGATE_WORKLOAD_JOBS}"
  case "$guard_value" in ''|*[!0-9]*) guard_value=$HARDGATE_WORKLOAD_JOBS ;; esac
  if ! [ "$guard_value" -ge 1 ] 2>/dev/null || ! [ "$guard_value" -le "$HARDGATE_WORKLOAD_JOBS" ] 2>/dev/null; then guard_value=$HARDGATE_WORKLOAD_JOBS; fi
  export "$guard_key=$guard_value"
done
unset CARGO_MAKEFLAGS guard_key guard_value
export MAKEFLAGS="-j$HARDGATE_WORKLOAD_JOBS"
