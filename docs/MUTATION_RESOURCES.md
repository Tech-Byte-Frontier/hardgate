# Workload and mutation resources
...
## Native mutation isolation

Mutation-report ingestion does not start mutation test commands.
...
Cgroup availability discounts clean disk page cache, while retaining anonymous memory, tmpfs, dirty pages, writeback, and kernel memory in the working estimate. Hard limits still account for all memory.
...
An inherited complete CLI boundary is reused rather than launching a sibling
scope that escapes its aggregate limits. Direct library mutation commands need
managed containment when no verified boundary is inherited. Missing containment,
manager failures and evidence errors fail closed; commands are not retried without
their cap.

## macOS and recovery

macOS does not currently have an enforced workload backend. The CLI refuses
workloads rather than silently substituting worker hints and process-group cleanup
for aggregate CPU and memory enforcement.
